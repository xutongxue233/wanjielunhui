...
const API_BASE = import.meta.env.VITE_CONTENT_API_BASE || '/api/v1/content';
const REMOTE_CONTENT_ENABLED = import.meta.env.VITE_ENABLE_CONTENT_API === 'true' || Boolean(import.meta.env.VITE_CONTENT_API_BASE);
...
async function contentRequest<T>(
  ...
  if (!REMOTE_CONTENT_ENABLED) {
    throw new Error('CONTENT_API_DISABLED');
  }
