...
import { getStoryNode, STORY_CHAPTERS, getChapterNodeCount, hasChapterData } from '../../data/stories';
import type { StoryNode, StoryChoice, StoryCondition, PlayerAttributes } from '../../types';
...
const evaluateValue = (
  actual: unknown,
  operator: StoryCondition['operator'],
  expected: StoryCondition['value']
): boolean => {
            ...
            >
              ...
              {hasData && (
                <div className="mt-2 text-xs text-muted">
                  内容量：约 {getChapterNodeCount(chapter.id)} 个剧情节点
                </div>
              )}
...
const ChapterCompleteView: React.FC<{
  ...
  onContinue: () => void;
  onBackToCultivation: () => void;
  hasNextChapter: boolean;
}> = ({ chapterName, rewards, onContinue, onBackToCultivation, hasNextChapter }) => {
  return (
    <div className="h-full flex flex-col items-center justify-center p-6 text-center">
      ...
      <div className="space-y-3 w-full max-w-xs">
        ...
        <Button variant="secondary" className="w-full" onClick={onBackToCultivation}>
          返回修炼
        </Button>
      </div>
    </div>
...
export const StoryPanel: React.FC<{ onReturnToCultivation?: () => void }> = ({ onReturnToCultivation }) => {
  ...
  const startStoryBattle = useGameStore((state) => state.startStoryBattle);
  ...
  const evaluateCondition = useCallback((condition: StoryCondition) => {
    if (!player) return false;

    switch (condition.type) {
      case 'flag': {
        const originKey = condition.key.startsWith('origin_')
          ? condition.key.slice('origin_'.length)
          : null;
        const actual = originKey ? player.origin === originKey : storyProgress.flags[condition.key];
        return evaluateValue(actual, condition.operator, condition.value);
      }
      case 'realm':
        if (condition.key === 'level') {
          return evaluateValue(player.realm.level, condition.operator, condition.value);
        }
        return false;
      case 'attribute': {
        const actual = (player.attributes as unknown as Record<string, number | undefined>)[condition.key];
        return evaluateValue(actual, condition.operator, condition.value);
      }
      case 'item': {
        const quantity = player.inventory
          .filter((entry) => entry.item.id === condition.key)
          .reduce((sum, entry) => sum + entry.quantity, 0);
        return evaluateValue(quantity, condition.operator, condition.value);
      }
      default:
        return false;
    }
  }, [player, storyProgress.flags]);

  const evaluateConditions = useCallback((conditions?: StoryCondition[]) => {
    if (!conditions || conditions.length === 0) return true;
    return conditions.every(evaluateCondition);
  }, [evaluateCondition]);

  const resolveStoryNode = useCallback((chapterId: string, nodeId: string) => {
    if (!player) return null;

    let currentNodeId: string | undefined = nodeId;
    let safety = 0;

    while (currentNodeId && safety < 20) {
      const node = getStoryNode(chapterId, currentNodeId, player.origin);
      if (!node) return null;
      ...
      const filteredChoices = node.choices.filter((choice) => evaluateConditions(choice.conditions));
      return {
        ...node,
  ...
  useEffect(() => {
    ...
    const node = resolveStoryNode(currentChapterId, currentNodeId);
    ...
    effects.forEach((effect) => {
      switch (effect.type) {
        case 'modify_attribute':
          modifyAttribute(effect.target as keyof PlayerAttributes, effect.value as number);
          break;
        case 'set_flag':
  ...
  const handleStartStoryBattle = () => {
    if (!player || !currentNode || currentNode.type !== 'battle') return;
    ...
    if (storyDungeon) {
      enemies = storyDungeon.enemyIds
        ...
        .filter((enemy): enemy is Combatant => enemy !== null);
    ...
    if (enemies.length === 0) {
      ...
      totalExp = enemies.length * 30;
      totalStones = enemies.length * 10;
    }

    startStoryBattle({
      chapterId: storyProgress.currentChapterId,
      nodeId: currentNode.id,
      nextNodeId: currentNode.nextNodeId,
      battleId: currentNode.battleId,
    });
  ...
  if (storyBattleState && battle) {
    return (
      <div className="h-full flex flex-col rounded-lg overflow-hidden story-panel">
        {renderHeader()}
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {currentNode && (
            <div className="rounded-lg border border-primary/20 bg-surface/60 p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-primary/80 mb-2">剧情战斗</p>
              {currentNode.speaker && (
                <p className="text-sm text-primary font-medium mb-1">[{currentNode.speaker}]</p>
              )}
              <p className="text-sm text-foreground/90 leading-7">{currentNode.content}</p>
            </div>
          )}
          <CombatPanel />
        </div>
  ...
  if (viewMode === 'complete') {
      return (
        <ChapterCompleteView
          ...
          onBackToCultivation={() => onReturnToCultivation?.()}
          ...
          {currentNode.type === 'choice' && currentNode.choices && !isTyping ? (
            ...
            <div className="text-center">
              ...
              <Button
                className="w-full"
                variant="primary"
                onClick={handleStartStoryBattle}
              >
                开始战斗
              </Button>
            </div>
