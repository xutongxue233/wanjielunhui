...
function isAutomationMode(): boolean {
  ...
  return new URLSearchParams(window.location.search).has('autotest');
}
...
export function useGameLoop() {
  ...
  const processOfflineRewards = useCallback(() => {
    ...
    // 新角色刚创建时不应吞入旧存档的离线收益。
    if (now - latestPlayer.createdAt < 60_000) {
  ...
  const tick = useCallback(() => {
    ...
    const deltaTime = Math.max(0, (now - lastTickRef.current) / 1000);
    lastTickRef.current = now;
  ...
  useEffect(() => {
    ...
    if (phase !== 'playing' || isAutomationMode()) return;

    const interval = setInterval(tick, TICK_INTERVAL);
    return () => clearInterval(interval);
  }, [tick, phase, isPaused, player?.id]);
