...
function createInitialPermanentTalents(): Record<string, PermanentTalentNode> {
  return Object.fromEntries(
    Object.values(PERMANENT_TALENTS).map((talent) => [
      talent.id,
      {
        ...talent,
        effects: talent.effects.map((effect) => ({ ...effect })),
        requires: [...talent.requires],
      },
    ])
  ) as Record<string, PermanentTalentNode>;
}

function createInitialRoguelikeState() {
  return {
    reincarnationCount: 0,
    destinyPoints: 0,
    totalDestinyPointsEarned: 0,
    permanentTalents: createInitialPermanentTalents(),
    currentRun: null,
    dailyEntries: {},
...
export const useRoguelikeStore = create<RoguelikeStore>()(
  persist(
    immer((set, get) => ({
      ...
      resetRoguelike: () => {
        set((state) => {
          Object.assign(state, createInitialRoguelikeState());
        });
      },
