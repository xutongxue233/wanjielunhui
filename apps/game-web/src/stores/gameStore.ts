...
import type { GamePhase, GameSettings, StoryProgress, RoguelikeState, PlayerAttributes, StoryEffect } from '../types';
import { STORY_CHAPTERS, checkChapterUnlock, getStoryNode } from '../data/stories';
...
interface GameState {
  ...
  // 剧情战斗状态
  storyBattleState: StoryBattleState | null;
  startStoryBattle: (battle: StoryBattleState) => void;
  resolveStoryBattle: (victory: boolean) => void;
  clearStoryBattle: () => void;
...
function applyStoryEffects(effects: StoryEffect[] | undefined) {
  ...
  effects.forEach((effect) => {
    switch (effect.type) {
      case 'modify_attribute':
        playerStore.modifyAttribute(effect.target as keyof PlayerAttributes, effect.value as number);
        break;
      case 'set_flag':
        gameStore.setStoryFlag(effect.target, effect.value);
        break;
      default:
        break;
    }
...
export const useGameStore = create<GameState>()(
  persist(
    immer((set, get) => ({
          ...
          for (const reward of chapter.rewards) {
            switch (reward.type) {
              case 'item': {
                ...
                const equipmentDef = getEquipmentById(reward.target);
                if (equipmentDef) {
                  playerStore.addItem({
                    item: {
                      id: equipmentDef.id,
      ...
      startStoryBattle: (battle) => {
        set((state) => {
          state.storyBattleState = battle;
        });
      },

      resolveStoryBattle: (victory) => {
        const storyBattleState = get().storyBattleState;
        const player = usePlayerStore.getState().player;

        if (!storyBattleState) {
          useCombatStore.getState().clearBattle();
          return;
        }
        ...
        const storyNode = getStoryNode(
          storyBattleState.chapterId,
          storyBattleState.nodeId,
          player.origin
        );
        ...
        if (storyNode) {
          get().addStoryEntry({
            id: storyNode.id,
            content: storyNode.speaker ? `[${storyNode.speaker}] ${storyNode.content}` : storyNode.content,
          });
        }

        applyStoryEffects(storyNode?.effects);

        if (storyBattleState.nextNodeId) {
          get().setCurrentNode(storyBattleState.chapterId, storyBattleState.nextNodeId);
          return;
        }

        get().completeChapter(storyBattleState.chapterId);
      },
