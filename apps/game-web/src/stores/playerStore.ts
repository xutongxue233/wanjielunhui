...
import { computeBreakthroughRate, computeCultivationSpeed } from '@wanjie/game-rules';
...
function createInitialAttributes(origin: OriginType, spiritualRoot: SpiritualRoot): PlayerAttributes {
  ...
  const base: PlayerAttributes = {
    ...
    cultivationSpeed: computeCultivationSpeed({
      baseSpeed: 1,
      spiritualRootBonus: rootBonus,
    }),
    comprehension: 10,
    luck: 10,
    karma: 0,
...
function createPlayer(name: string, gender: 'male' | 'female', origin: OriginType): Player {
  ...
  const startingTechniques = (originConfig.startingBonus.techniques || [])
    .map((techniqueId) => getTechniqueById(techniqueId))
    .filter((technique): technique is Technique => Boolean(technique));
  const startingInventory: InventoryItem[] = (originConfig.startingBonus.items || [])
    .map<InventoryItem | null>((entry) => {
  ...
  return {
    ...
    activeTechniqueId: startingTechniques[0]?.id || null,
    skills: [],
  ...
  persist(
    immer((set, get) => ({
      ...
      attemptBreakthrough: () => {
        ...
        const successRate = computeBreakthroughRate({
          baseRate,
          comprehension: player.attributes.comprehension,
