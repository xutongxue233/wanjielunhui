...
import {
  ...
  isStunned,
  consumeControlEffects,
  selectSkillAI,
  applyStatusEffect,
  createStatusEffectFromSkillEffect,
  getStatusResistance,
} from '../data/combat';
...
function isHostileSkill(skill: CombatSkill): boolean {
  if (skill.type === 'attack' || skill.type === 'ultimate') {
    return true;
  }

  const hasHostileEffect = skill.effects.some(effect =>
    effect.type === 'damage' || effect.type === 'debuff' || effect.type === 'dot'
  );
  const hasHelpfulEffect = skill.effects.some(effect =>
    effect.type === 'heal' || effect.type === 'buff' || effect.type === 'hot'
  );

  if (hasHostileEffect && !hasHelpfulEffect) {
    return true;
  }
...
function getSkillTargets(
  ...
  const targetPool = isHostileSkill(skill)
    ? (actor.isAlly ? battle.enemies : battle.allies)
    : (actor.isAlly ? battle.allies : battle.enemies);
  const aliveTargets = targetPool.filter(combatant => combatant.isAlive);
  ...
  if (skill.targetType === 'random') {
    const target = aliveTargets[Math.floor(Math.random() * aliveTargets.length)];
    return target ? [target] : [];
  }

  const resolvedTarget = aliveTargets.find(combatant => combatant.id === primaryTarget.id);
  return resolvedTarget ? [resolvedTarget] : [aliveTargets[0]];
}

function getEffectLogMessage(effect: SkillEffectConfig, targetName: string): string {
  const effectName = effect.statusType ? (STATUS_EFFECT_LABELS[effect.statusType] || effect.statusType) : '效果';

  switch (effect.type) {
    case 'heal':
      return `${targetName}恢复${effect.value}点生命`;
    case 'damage':
      return `${targetName}额外受到${effect.value}点伤害`;
    case 'buff':
      return `${targetName}获得${effectName}效果`;
    case 'debuff':
      return `${targetName}受到${effectName}效果`;
    case 'dot':
      return `${targetName}被施加持续伤害效果`;
    case 'hot':
      return `${targetName}被施加持续治疗效果`;
    default:
      return `${targetName}受到额外效果影响`;
  }
}

function getEffectResistLogMessage(effect: SkillEffectConfig, targetName: string): string {
  const effectName = effect.statusType ? (STATUS_EFFECT_LABELS[effect.statusType] || effect.statusType) : '效果';
  return `${targetName}抵抗了${effectName}效果`;
}

function shouldForceAutomationProc(): boolean {
  if (typeof window === 'undefined') {
    return false;
  }

  return window.__WANJIE_AUTOMATION_FORCE_PROC__ === true;
}
...
export const useCombatStore = create<CombatState>()(
  persist(
    immer((set, get) => ({
      ...
      nextTurn: () => {
        const { battle, currentDungeonId } = get();
      ...
      executeAction: (skillId, targetId) => {
        ...
        set((state) => {
          ...
          const targets = getSkillTargets(state.battle, actor, skill, target);
          let performedPrimaryAction = false;

          const canDealDamage = skill.damageMultiplier > 0 && (skill.type === 'attack' || skill.type === 'ultimate');
          if (canDealDamage) {
          ...
          } else if (skill.type === 'support' && skill.damageMultiplier > 0) {
            targets.forEach(t => {
              const healAmount = calculateHeal(actor, t, skill);
          ...
          skill.effects.forEach(effect => {
            const forceAutomationProc = shouldForceAutomationProc();
            const chance = effect.chance ?? 1;
            const effectTargets = (effect.type === 'heal' || effect.type === 'buff' || effect.type === 'hot') &&
              (skill.type === 'attack' || skill.type === 'ultimate')
              ? [actor]
              : targets;

            effectTargets.forEach(effectTarget => {
              if (!forceAutomationProc && Math.random() > chance) return;

              if (effect.type === 'damage') {
                effectTarget.hp = Math.max(0, effectTarget.hp - effect.value);
                if (effectTarget.hp === 0) {
                  effectTarget.isAlive = false;
                }
              } else if (effect.type === 'heal') {
                effectTarget.hp = Math.min(effectTarget.maxHp, effectTarget.hp + effect.value);
              } else {
                const statusEffect = createStatusEffectFromSkillEffect(effect);
                if (!statusEffect) return;

                if (statusEffect.type === 'debuff') {
                  const resistance = getStatusResistance(effectTarget, statusEffect);
                  if (resistance >= 1 || Math.random() < resistance) {
                    state.battle!.logs.push({
                      turn: state.battle!.turn,
                      actorId: actor.id,
                      actorName: actor.name,
                      action: getEffectResistLogMessage(effect, effectTarget.name),
                    });
                    return;
                  }
                }

                applyStatusEffect(effectTarget, statusEffect as StatusEffect);
              }
              ...
              state.battle!.logs.push({
                ...
                actorId: actor.id,
                actorName: actor.name,
                action: getEffectLogMessage(effect, effectTarget.name),
              });
            });
          });

          if (!performedPrimaryAction && skill.effects.length === 0) {
            state.battle.logs.push({
              turn: state.battle.turn,
              actorId: actor.id,
              actorName: actor.name,
      ...
      getAvailableTargets: (skill) => {
        ...
        return targetPool.filter(c => c.isAlive);
      },
